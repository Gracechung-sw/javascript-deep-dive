...
[실행 컨텍스트]()에서 살펴본 바와 같이 함수를 호출하면 함수 코드가 평가되어 함수 실행 컨텍스트가 생성된다. 이 때 생성된 함수 실행 컨텍스트는 실행 컨텍스트 스택 (콜 스택. Call Stack)에 push되고, 함수 코드가 실행된다. 그리고 함수 실행이 종료되면 call stack에서 pop된다. 


그리고 call stack이 있는데, 함수 실행 순서를 기억한다. 이 때, JS Call stack는 **하나의 싱글(단 하나의) 컨텍스트 스택을 가진다.** 이건 어떤 의미냐면, 함수를 실행할 수 있는 창구가 단 하나이기 때문에 동시에 2개 이상의 함수는 실행할 수 없다는 것을 의미하며, 실행 컨텍스트 스택의 최상위 요소인 '실행 중인 실행 컨텍스트'를 제외한 모든 실행 컨텍스트는 대기 중인 태스크 들이다.    
**즉, JS는 한번에 하나의 일만 처리 Single Thread 방식으로 동작한다.** 
그래서 JS는 기본적으로 **동기적(현재 실행 중인 태스크가 종료될 때까지 다음에 실행될 태스크가 대기중인 방식)**으로 진행되며, 처리에 시간이 걸리는 태스크를 실행하는 경우, 블로킹(Blocking) 작업 중단이 발생한다. 

## 동기와 비동기의 차이점
- 동기는 앞서 말했던 것 처럼 '현재 실행 중인 태스크가 종료될 때까지 다음에 실행될 태스크가 대기중인 방식'인 것이며
- 비동기는 '현재 실행 중인 태스크가 종료 되지 않은 상태라 해도 다음 태스크를 곧바로 실행하는 방식'을 말한다. 
